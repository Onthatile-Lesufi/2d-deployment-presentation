...
function Admin() {
  ...
  const flagProduct = async() => {
    ...
    try {
      const res = await axios.put(`${process.env.REACT_APP_API_URL}/api/products/updateStatus/${id}`, {
        status
      });
      setIsFlagged(true);
    } catch (error) {
  ...
  const addToCart = async () => {
    try {
      const res = await axios.put(
        `${process.env.REACT_APP_API_URL}/api/users/cart/${user.email}`,
        {
          productID: id,
        }
      );
  ...
  const addToWishlist = async () => {
    try {
      const res = await axios.put(
        `${process.env.REACT_APP_API_URL}/api/users/wishlist/${user.email}`,
        {
          productID: id,
        }
      );
