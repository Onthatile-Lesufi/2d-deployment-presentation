...
const ReviewContainer = ({productId}) => {
    ...
    async function CheckCredentials() {
      try {
        const _user = await axios.get("https://dv200-deployment-presentation-4af1d27ec0f4.herokuapp.com/api/users/logged", {
    ...
    async function CaptureReviewInformation () {
        if (itemRating) {
            try {
                ...
                const _tempResult = await axios.post(`https://dv200-deployment-presentation-4af1d27ec0f4.herokuapp.com/api/products/${productId}/review/post`, {
                    _rating: parseFloat(itemRating),
                    _productReview: reviewText,
                    _user: user.email
                });
                const _rating = await axios.put(`https://dv200-deployment-presentation-4af1d27ec0f4.herokuapp.com/api/products/${productId}/rating`);
            } catch (error) {
                console.log(error);
            }
    ...
    async function GrabReviews() {
        try {
            const _reviews = await axios.get(`https://dv200-deployment-presentation-4af1d27ec0f4.herokuapp.com/api/products/${productId}/reviews`);
